...
async def export_cover_letter(request: CoverLetterRequest) -> str:
    """
    Generate a cover letter based on the job description keywords and resume profile.
    Args:
        request (CoverLetterRequest): The request containing jd_keywords, resume_profile, resume_report, and pages.
    Returns:
        str: The generated cover letter as a string.
    """
    if not request.jd_keywords or not request.resume_profile:
        raise HTTPException(status_code = 400, detail="Both job description keywords and resume profile are needed.")
    try:
        cover_letter = generate_cover_letter(request.jd_keywords, request.resume_profile, request.resume_report, model, request.pages)
        if not cover_letter:
...
class CoverEmailRequest(BaseModel):
    jd_keywords: JDKeyWords
    resume_profile: ResumeProfile
    resume_report: ResumeReport

@router.post("/cover-email")
async def export_cover_email(request: CoverEmailRequest) -> str:
    """
    Generate a cover email based on the job description keywords and resume profile.
    Args:
        request (CoverEmailRequest): The request containing jd_keywords, resume_profile, and resume_report.
    Returns:
    ...
    if not request.jd_keywords or not request.resume_profile:
        raise HTTPException(status_code = 400, detail="Both job description keywords and resume profile are needed.")
    try:
        cover_email = generate_cover_email(request.jd_keywords, request.resume_profile, request.resume_report, model)
        if not cover_email:
